/**
 * Browser copy — keep in sync with scripts/lib/path-smooth.mjs
 */
...
export function ellipsePathFromPoints(points) {
  ...
  return (
    ...
    ` C ${round(cx - kx)} ${round(cy + ry)} ${round(cx - rx)} ${round(cy + ky)} ${round(cx - rx)} ${round(cy)}` +
    ` C ${round(cx - rx)} ${round(cy - ky)} ${round(cx - kx)} ${round(cy - ry)} ${round(cx)} ${round(cy - ry)} Z`
  );
}
...
export const DEFAULT_ZONE_SMOOTH = {
  ...
  'zone-1': { mode: 'spline', chaikin: 0, tension: 0.45, simplify: 6 },
};
